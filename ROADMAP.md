...
Phase 6 — The MCP server. Now MCP enters. Write a Python MCP server on the Pi that exposes tools: get_recent_detections, get_snapshot, get_cat_status ("inside" / "outside" based on direction of travel or last seen), get_stats. Run it as an SSE server on the Pi's local IP. End state: from your laptop's Claude Desktop, you can ask "did Mochi go out today?" and get a real answer.
